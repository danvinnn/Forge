...
export const FIELD_GUIDE: Record<ExtractionField, string> = {
  ...
  "dimensions.leadThicknessMm":
    "ONLY where dimensions.mounting is 'through-hole' AND the lead is a flat stamped pin rather than a round wire: the THICKNESS of that pin, in millimetres, as a min and max. On a DIP-style outline this is the dimension usually lettered C, e.g. '0.20 - 0.36', and it is the smaller of the two the drawing gives for the lead; dimensions.leadWidthMm is the other one, usually lettered b, seen from above. Report both and neither as a diameter. Do not report b2 or any shoulder dimension above the pin. Null for a round lead, which has a diameter instead, and null for a surface-mount package.",
