...
/**
 * Remembers one install-scoped answer, under its OWN field name.
 *
 * A single-value version of this stored any install answer as the lead span, so
 * answering the foot overwrote the span with it and the next flat pack was built
 * from the wrong number.
 *
 * Writes the same `forge.install.*` key `src/app/page.tsx` reads, deliberately:
 * they are the same two numbers and a customer who answered one mid-parse must
 * not be asked again on the other screen.
 */
export function rememberInstallAnswer(field: string, value: number): void {
  const held = store();
  if (!held) return;
  try {
    held.setItem(`${INSTALL_KEY_PREFIX}${field}`, String(value));
  } catch {
    // Remembering is a convenience; failing to remember must not block a build.
  }
}
