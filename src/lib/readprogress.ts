 ...
 * 3. **THE LAST STAGE CANNOT COMPLETE ON A TIMER.** Every stage but the last
 *    advances on elapsed time; the last is closed by the fetch resolving, which
 *    is a real event. So the one transition that means "this worked" is never
 *    guessed.
 *
 ...
 *
 * ## The stage that only exists on one of the two paths
 *
 * A lookup and an upload are not the same request. An upload hands `/api/parse`
 * the bytes; a lookup makes `/api/lookup` go and FIND them first, walking the
 * resolver chain against vendor hosts, and that took up to 12 seconds until
 * 2026-09-04 and takes up to 45 now.
 *
 * Until that date this file described the model pipeline and nothing else, on
 * both paths. So a lookup spent its whole retrieval showing "Whole document to
 * the model", which had not started and had no document yet - and because the
 * schedule totalled ninety seconds while the request could now run to a hundred
 * and thirty-five, a perfectly normal lookup would reach the end of the schedule
 * and drop into the overrun creep, which is the shape reserved for a read that
 * has gone long. The bar would have called the common case abnormal.
 *
 * So retrieval is a stage of its own, and only on the path that pays for it.
 *
...
/**
 * Typical seconds for the resolver chain to find a datasheet, on the lookup path.
 *
 * MEASURED, not proportioned like the rest of this schedule. 96 resolver hits
 * across the retrieval hold-out and the live coverage corpus on 2026-09-04, at
 * the 45 second chain budget the product now runs:
 *
 *     median 1.1s    mean 3.2s    p75 3.0s    p90 7.2s    max 30.0s
 *
 * The mean, rounded up. Not the median, because the distribution has a long
 * tail and the median would under-run three lookups in four; not the p90,
 * because a bar that over-estimates STALLS and a bar that under-estimates
 * CREEPS, and this file's whole posture is that creeping is the better failure.
 *
 * It is deliberately NOT `resolveChainBudgetMs()`. That is the ceiling a MISS
 * costs, and a schedule built from it would spend forty-five seconds crawling
 * through a fetch whose median is a second, and then jump.
 */
const RETRIEVAL_SECONDS = 4;

/**
 * @param retrieving true on the lookup path, where the datasheet must be found
 *   before anything can be read. False for an upload, which already has it.
 */
export function stagesFor(intent: Intent, options?: { retrieving?: boolean }): ReadStage[] {
  const third =
    intent === "spice"
  ...
  const finding: ReadStage[] = options?.retrieving
    ? [
        {
          name: "Finding the datasheet",
          seconds: RETRIEVAL_SECONDS,
          note: "Manufacturer sites first, in order, until one answers with the real document."
        }
      ]
    : [];
  return [
    ...
    { name: third.name, seconds: 48, note: third.note },
    {
      ...
      seconds: 20,
      note: "Independent readings are compared; disagreements are surfaced, never guessed through."
    }
  ];
}
